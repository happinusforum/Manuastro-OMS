...
const Accounting = () => {
  ...
  const handleToggleUser = async (targetUid) => {
    ...
    setAllowedUserIds(newIds);
    await setDoc(doc(db, 'settings', 'accounting_access'), { uids: newIds }, { merge: true });
  };

  // --- 3. PERMISSION LOGIC ---
  const canSeePayroll = userProfile?.role === 'admin' || userProfile?.role === 'hr';
  const canSeeTools = userProfile?.role === 'admin' || allowedUserIds.includes(userProfile?.uid);
  const canManageAccess = userProfile?.role === 'admin';
  ...
  if (isLoadingPerms) return <div className="p-10 text-center">Loading Permissions...</div>;
      ...
      <div className="max-w-6xl mx-auto mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2 text-lg">
            Manage payroll, billing, and inventory.
          </p>
        </div>
        ...
        {canManageAccess && (
          <button 
            ...
            className="flex items-center gap-2 bg-gray-900 dark:bg-gray-700 text-white px-5 py-2.5 rounded-xl font-bold shadow-lg hover:bg-black transition-all"
          >
            <ShieldCheck size={18} /> Manage Access
          </button>
        )}
      </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg border border-gray-200 dark:border-gray-700 flex flex-col max-h-[80vh] animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar">
              <p className="text-sm text-gray-500 mb-4 bg-yellow-50 dark:bg-yellow-900/20 p-3 rounded-lg border border-yellow-200 dark:border-yellow-800">
                ...
                Select employees who can access <b>GST Invoice</b> & <b>Inventory</b>. Payroll remains restricted to HR & Admin.
              </p>

              <div className="space-y-2">
                {allUsers && allUsers.map(emp => {
                    // 🔥 SMART NAME LOGIC: displayName -> name -> email prefix
                    const userName = emp.displayName || emp.name || emp.fullName || emp.email?.split('@')[0] || "User";
                    ...
                    return (
                      <label key={emp.id} className="flex items-center justify-between p-3 rounded-xl border border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer transition-colors">
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${allowedUserIds.includes(emp.id) ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-200 text-gray-500'}`}>
                            {initial}
                          </div>
                          <div>
                        ...
                        <div className={`w-6 h-6 rounded border flex items-center justify-center transition-all ${allowedUserIds.includes(emp.id) ? 'bg-indigo-600 border-indigo-600' : 'border-gray-300 dark:border-gray-500'}`}>
                        ...
                        <input 
                          type="checkbox" 
                          className="hidden" 
                          checked={allowedUserIds.includes(emp.id)} 
            ...
            <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/30 rounded-b-2xl text-right">
              <button onClick={() => setIsModalOpen(false)} className="px-6 py-2 bg-indigo-600 text-white rounded-lg font-bold shadow-md hover:bg-indigo-700">Done</button>
            </div>
          </div>
        </div>
      )}
