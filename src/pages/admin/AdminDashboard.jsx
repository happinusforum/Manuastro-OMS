...
import { Users, Clock, Shield, UserPlus, FileText, PieChart, ArrowRight } from 'lucide-react';
...
function AdminDashboard() {
    ...
    // 💡 Real-time calculation (Logic Unchanged)
    const adminStats = useMemo(() => {
        if (usersLoading || leavesLoading || !users || !leaves) {
            return [
                { title: 'Total Employees', value: '...', icon: <Users size={24} />, color: 'from-blue-500 to-blue-600', shadow: 'shadow-blue-200 dark:shadow-blue-900/20' },
                { title: 'Pending Requests', value: '...', icon: <Clock size={24} />, color: 'from-amber-500 to-amber-600', shadow: 'shadow-amber-200 dark:shadow-amber-900/20' },
                { title: 'System Admins', value: '...', icon: <Shield size={24} />, color: 'from-purple-500 to-purple-600', shadow: 'shadow-purple-200 dark:shadow-purple-900/20' },
            ];
        }

        const totalEmployees = users.length;
        const pendingLeaves = leaves.filter(l => l.status === 'Pending HR').length; 
        const totalAdmins = users.filter(u => u.role === 'admin').length;
        
        return [
            { title: 'Total Employees', value: totalEmployees, icon: <Users size={24} />, color: 'from-blue-500 to-blue-600', shadow: 'shadow-blue-200 dark:shadow-blue-900/20' },
            { title: 'Pending Requests', value: pendingLeaves, icon: <Clock size={24} />, color: 'from-amber-500 to-amber-600', shadow: 'shadow-amber-200 dark:shadow-amber-900/20' },
            { title: 'System Admins', value: totalAdmins, icon: <Shield size={24} />, color: 'from-purple-500 to-purple-600', shadow: 'shadow-purple-200 dark:shadow-purple-900/20' },
        ];
    ...
    return (
        <div className="min-h-screen bg-gray-50/50 dark:bg-gray-900 p-6 md:p-8 transition-colors duration-300">
            ...
            <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-extrabold text-gray-800 dark:text-white tracking-tight">Admin Overview</h1>
                    <p className="text-gray-500 dark:text-gray-400 mt-2 font-medium">
                        Good to see you, <span className="text-indigo-600 dark:text-indigo-400 font-bold">{currentUser?.email?.split('@')[0]}</span> 👋
                    </p>
                </div>
            ...
            {(usersLoading || leavesLoading) ? (
                <div className="flex justify-center items-center h-64">
                    <LoadingSpinner message="Analysing data..." />
                </div>
            ) : (
                <motion.div 
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {adminStats.map((stat, index) => (
                            <motion.div 
                                ...
                                whileHover={{ y: -5 }}
                                className={`relative overflow-hidden bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-xl ${stat.shadow} border border-gray-100 dark:border-gray-700 transition-colors`}
                            >
                                ...
                                {/* Decorative Circle */}
                                <div className={`absolute -bottom-4 -right-4 w-24 h-24 rounded-full opacity-10 bg-gradient-to-br ${stat.color}`}></div>
                            </motion.div>
                        ))}
                    </div>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
                            ...
                            >
                                ...
                                <h4 className="font-bold text-gray-800 dark:text-gray-100">Manage Users</h4>
                                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Add, edit or remove employees</p>
                            </button>
                            ...
                            >
                                ...
                                <h4 className="font-bold text-gray-800 dark:text-gray-100">Leave Approvals</h4>
                                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Review pending leave requests</p>
                            </button>
                            ...
                            >
                                ...
                                <h4 className="font-bold text-gray-800 dark:text-gray-100">Payroll System</h4>
                                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Generate reports & slips</p>
                            </button>
